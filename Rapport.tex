...
Chiefly, there are two kind of external forces can give out perceptible influence on our simulative sailboats: the wind and the wave. Since air and water are in touch with the sailboats directly, there is no force from the land. Among all the forces given by air and the water, the friction and damping are insignificant. Therefore we talk about our method for simulating the the wind and the wave.\\

The situation of wind in the real world is really complicated. Even the most experienced meteorologist cannot predict the variation of wind for a 100\% accuracy. In our case, it is next to impossible to simulate the wind in an open sea area. Even if we generate randomly the force, this makes no sense except give out a confusing motion of the sailboat under irregular wind. Conversely, if we suppose the wind as constant, we don't really lost the similarity to real word. Meanwhile, we get a lot of time saved as well as the illustration simplified.\\

For the part of waves, we studied the existing work by Mr.Ponte about the analytical solutions of plausible oceanic flows.The mathematic model of multiple 3D eddies is given as \ref{eq:eddies}. \\
\begin{equation}
\begin{aligned}
...
This model and simulation of eddies enlightened us. . However, we find that the sailboat will always work on the sea level and we do not need a 3d simulation of the water. Therefore we chose to ignore the wave and Waves are not simulated eventually.\\

In brief, we analyzed the complexity and compared the result of different choices, and finally we assume that the wind is constant and the wave is static.
...
\subsubsection{The Sea Plot API}
A separate module called seaplotapi was used to provide the drawing functions. Each functions within gives a Numpy array (vector or matrix), which is called in the main file for each drawn objects. 
\subsection{Result}
